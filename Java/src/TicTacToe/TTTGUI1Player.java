...
/**
 * Write a description of class TTTGUI1Player here.
 *
 * @author (your name)
 * @version (a version number or a date)
 */
public class TTTGUI1Player extends JFrame implements ActionListener, KeyListener{
    ...
    public void takeTurn(){
        ArrayList<int[]> freeTiles = new ArrayList<int[]>();
        
        for (int r = 0; r < theTiles.length; r++){
            for (int c = 0; c < theTiles.length; c++){
                if (theTiles[r][c].getValue() == 0){
                    int[] freeCoord = {r, c};
                    freeTiles.add(freeCoord);
                }
            }
        }
        
        if (freeTiles.size() > 1){
        int[] chosenCoord = freeTiles.get(rand.nextInt(freeTiles.size() - 1));
        theTiles[chosenCoord[0]][chosenCoord[1]].setSelected(true);
    ...
    public static void main(String[] args){
        TTTGUI1Player frame = new TTTGUI1Player();
